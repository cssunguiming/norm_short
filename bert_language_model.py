...
class BertLM(nn.Module):
    ...
    def __init__(self, Bert_Traj_Model, token_size, head_n=12, d_model=768, N_layers=12, dropout=0.1):
        ...
        # self.bert = Bert_Traj_Model(token_size=token_size, head_n=head_n, d_model=d_model, N_layers=N_layers, dropout=dropout)
        self.bert = Bert_Traj_Model
        self.next_sen_pred = NextSentencePredict(d_model=d_model)
        self.masked_lm = Masked_LM(token_size=token_size, d_model=d_model)

        for p in self.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)

    def forward(self, x, seg_label, time):

        x = self.bert(x, seg_label, time)
        logit1 = self.next_sen_pred(x)
        logit2 = self.masked_lm(x)
        return logit1.view(-1, logit1.size(-1)), logit2.view(-1, logit2.size(-1))

    def save_bert(self, Epoch, file_path="./pretrain/"):
        output_path = file_path + "bert_trained_ep%d.pth" % Epoch
        torch.save(self.bert.state_dict(), output_path)
        print("EP:%d Bert Saved on:" % Epoch, output_path)
        return output_path
