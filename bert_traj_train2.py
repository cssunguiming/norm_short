...
def predict_train_epoch(epoch, model, train_data, train_queue, optimizer, device, batch_size):
    ...
    for i, batch_queue in enumerate(train_queue):
        ...
        max_place = max([len(train_data[u][id]['tim']) for u,id in batch_queue]) + 1 # 最后一位用于存信息
...
def predict_valid_epoch(epoch, model, valid_data, valid_queue, optimizer, device, batch_size):
    ...
    with torch.no_grad():
        for i, batch_queue in enumerate(valid_queue):
            ...
            max_place = max([len(valid_data[u][id]['loc']) for u,id in batch_queue]) + 1
...
def main(Epoch=400, Bert_Pretrain=False, Pretrained=False, Batch_size=10, log='predict'):
    ...
    if Bert_Pretrain:
        ...
        Bert = Bert_Traj_Model(token_size=len(dataset_4qs['vid_list']), head_n=1, d_model=240, N_layers=6, dropout=0.5)
        Bert.load_state_dict(torch.load('./pretrain/bert_trained_ep14.pth')) 
    else: 
        print("Create New Bert")      
        Bert = Bert_Traj_Model(token_size=len(dataset_4qs['vid_list']), head_n=1, d_model=128, N_layers=2, dropout=0.5)

    print("Get Predict Model")
    model = Predict_Model(Bert, token_size=len(dataset_4qs['vid_list']), head_n=1, d_model=128, N_layers=2, dropout=0.5) 
    if Pretrained:
