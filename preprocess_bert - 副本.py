...
class Data_deepmove(object):
    def __init__(self, trace_min=10, global_visit=10, hour_gap=72,
        ...
        self.time_interval_count = {}
        self.time_interval_x = []
    ...
    def filter_users_by_length(self):
        ...
        for u in pick3:
            ...
            for i, record in enumerate(data):
                ...
                # print("tid", tid)
                # # print("exit in filter _ users by length")
                # # exit()
    ...
    def tid_list_48(tmd):
        ...
        return tid + 1
    @staticmethod
    def tid_interval_list(tim):
        time_s = int(time.mktime(time.strptime(tim, "%Y-%m-%d %H:%M:%S")))
        return time_s
    ...
    def prepare_neural_data(self):
        ...
        for u in self.uid_list:
            ...
            for sess_id in sessions:
                ...
                a = [self.tid_interval_list(x[1]) for x in sessions[sess_id]]
                time_interval = ((np.array(a[1:])-np.array(a[:-1]))/60).astype(np.int32)
               
                
                for i in time_interval:
                    self.time_interval_x.append(i)
                    if i in self.time_interval_count.keys():
                        self.time_interval_count[i] += 1
                    else:
                        self.time_interval_count[i] = 1

                sessions_tran[sess_id] = [[self.vid_list[x[0]][0], self.tid_list_48(x[1]), self.tid_interval_list(x[1])] for x in sessions[sess_id]]
            ...
            self.data_neural[self.uid_list[u][0]] = {
                ...
                'train_loc': train_loc
                # 'time_internal': self.time_interval_count
                # 'explore': location_ratio, 'entropy': entropy, 'rg': rg
            }
        # print(len(self.time_interval_count))
        # print("max",maxtime)
        # print("min",mintime)
        # print("\nn", n)
        # exit()
        # user_list = list(self.data_neural.keys())
        # n = 0
        # for user in user_list:
        #     n += self.data_neural[user]['train_loc']



    def save_variables(self):
        n = 0
        for i in self.time_interval_count.keys():
            n+=self.time_interval_count[i]
        print("\n",n,"\n")
        ...
        foursquare_dataset = {
            ...
            'time_internal': self.time_interval_count,
            'time_interval_x': self.time_interval_x
        }
